...
from torch_geometric.nn import global_mean_pool, GlobalAttention, GCNConv, SGConv
...
class slg_advanced(nn.Module):
    def __init__(self, node_dim: int, edge_dim: int, out_dim: int, cfg: DictConfig):
        ...
        self.dataset_name = cfg.data.get('name', 'default')
        ...
        # H^(2) Construction: GCN over L_2(G) or Twists
        self.use_gcn = cfg.model.get('use_gcn', False)
        if self.use_gcn:
            self.gcn = GCNConv(self.hidden_dim, self.hidden_dim)

        # SGN implementation (SGC -> BatchNorm -> ReLU)
        self.use_sgn = cfg.model.get('use_sgn', False)
        if self.use_sgn:
            self.sgc = SGConv(self.hidden_dim, self.hidden_dim, K=2)
            self.sgn_bn = nn.BatchNorm1d(self.hidden_dim)

        # Configuration for specific dimensional additions (Twists)
        self.topology_info = cfg.model.get('topology_info', True)
        self.use_in_out = cfg.model.get('use_in_out', False) # Encodes degree
        
        # Determine the dimension of the final hidden representation before readout
        self.h_final_dim = self.hidden_dim
        if self.topology_info:
            self.h_final_dim += 1
        if self.use_in_out:
            self.h_final_dim += 1
            
        # Readout Configuration (H_G)
        self.readout_type = cfg.model.get('readout_type', 'mean')
        self.readout_dim = self.h_final_dim
        
        if self.readout_type == 'attention':
            self.att_gate = nn.Sequential(
                nn.Linear(self.h_final_dim, self.h_final_dim // 2),
                nn.ReLU(),
                nn.Linear(self.h_final_dim // 2, 1)
            )
            ...
            self.st_d_model = self.h_final_dim
    ...
    def forward(self, data):
        ...
        if num_l2 > 0:
            ...
            # Equation: ST_1(h^(0)_u1, h^(0)_u2) + h^(0)_e
            # ==========================================
            ...
            # ==========================================
            # 3. Compute h_A (H^(1))
            # Equation: ST_2(h^(1)_e, h^(1)_f) + 1/4 \sum h^(0)_ui
            # ==========================================
            ...
            # ==========================================
            # NEW: SGN Transformation (SGC -> BatchNorm -> ReLU)
            # ==========================================
            if self.use_sgn:
                if hasattr(data, 'l2_edge_index') and data.l2_edge_index.size(1) > 0:
                    H1 = self.sgc(H1, data.l2_edge_index)
                    
                # BatchNorm1d requires batch_size > 1 during training
                if H1.size(0) > 1 or not self.training:
                    H1 = self.sgn_bn(H1)
                
                H1 = F.relu(H1)

            # ==========================================
            # 4. Compute H^(2): Structure of L_2(G)
            # ==========================================
            # Option A: GCN over the L2 graph
            if self.use_gcn:
                if hasattr(data, 'l2_edge_index') and data.l2_edge_index.size(1) > 0:
                    H1 = F.relu(self.gcn(H1, data.l2_edge_index)) + H1
                else:
                    H1 = F.relu(H1) # Fallback if L2 graph has isolated nodes
            
            # Option B / Addition: Topology Info
            if self.topology_info:
                is_incident = ((u1 == v1) | (u1 == v2) | (u2 == v1) | (u2 == v2)).float().unsqueeze(-1)
                H1 = torch.cat([H1, is_incident], dim=-1)

            # Option C / Addition: L2 Degree Info (without message passing)
            if self.use_in_out:
                if hasattr(data, 'l2_edge_index') and data.l2_edge_index.size(1) > 0:
            ...
            H1 = torch.empty((0, self.h_final_dim), device=x.device)
            l2_batch = torch.empty((0,), dtype=torch.long, device=x.device)

        # ==========================================
        # 5. Readout Function (H_G)
        # ==========================================
        if H1.size(0) == 0:
            ...
            if self.readout_type == 'mean':
                ...
                if self.st_d_model > self.h_final_dim:
                    H1_padded = F.pad(H1, (0, self.st_d_model - self.h_final_dim))
                else:
                    H1_padded = H1
